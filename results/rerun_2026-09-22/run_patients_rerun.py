"""Drives the live, redesigned Drug Interaction Screen for the same 10 synthetic patients and reports the results.

Adapted from results/run_patients.py (read-only, not modified) for the
2026-09-22 redesign: the old page had a "Medication list" textarea and a
"Check interactions" button; the new page has an auto-focused medication
field with no button, adds one entry per Enter press, and runs the check
automatically 300ms after the list changes. This script adds each patient's
drugs one at a time by pressing Enter, then waits for the settled
POST /api/check response the same way the original script did: intercepted
directly, not scraped from the DOM, plus the rendered results text and a
...
MEDICATION_FIELD_LABEL = "Add a medication"
CHECK_ENDPOINT_MARKER = "/api/check"
SETTLE_TIMEOUT_MS = 30_000
DEBOUNCE_SETTLE_MS = 900  # comfortably over the app's 300ms debounce
...
class PatientResult:
    """One patient's captured outcome: the settled API response, the rendered panel text, and pass/fail."""
...
def find_medication_field(page: Page) -> Locator:
    """
    Takes the page to search.
    Locates the medication field by its accessible label.
    Gives the Playwright locator for that field.
    """
    return page.get_by_label(MEDICATION_FIELD_LABEL)


def find_results_panel(page: Page) -> Locator:
    """
    Takes the page to search.
    Locates the results section by its class, as seen in the page markup.
    Gives the Playwright locator for that section.
    """
...
def is_check_response(response: Response) -> bool:
    ...
    Checks whether its URL is the interaction-check endpoint.
    Gives True when it is, False otherwise.
    """
    return CHECK_ENDPOINT_MARKER in response.url


def add_one_entry(page: Page, text: str) -> None:
    """
    Takes the page and one medication line.
    Fills the medication field with it and presses Enter, the new UI's own way to add one entry.
    Gives nothing.
    """
    field = find_medication_field(page)
    field.fill(text)
    field.press("Enter")
...
def submit_one_patient(page: Page, patient_number: int, medications: str) -> PatientResult:
    ...
    Adds every drug with one Enter press each, waits for the settled /api/check response, and captures the panel text and a screenshot.
    Gives the PatientResult, marked failed with the raw error when anything times out or the last response is not 200.
    """
    screenshot_path = RESULTS_DIR / f"patient_{patient_number:02d}.png"
    responses: list[Response] = []
    page.on("response", lambda response: responses.append(response) if is_check_response(response) else None)
    try:
        for drug in [part.strip() for part in medications.split(",") if part.strip()]:
            add_one_entry(page, drug)
        page.wait_for_timeout(DEBOUNCE_SETTLE_MS)
        page.wait_for_function(
            "() => { const h = document.querySelector('.results__heading'); return h && h.textContent.trim().length > 0; }",
            timeout=SETTLE_TIMEOUT_MS,
        )
        panel_text = find_results_panel(page).inner_text()
        page.screenshot(path=str(screenshot_path), full_page=True)
        if not responses:
            return PatientResult(patient_number, medications, False, None, None, panel_text, "no /api/check response was observed", screenshot_path)
        return build_result_from_response(patient_number, medications, responses[-1], panel_text, screenshot_path)
    except PlaywrightTimeoutError as error:
        page.screenshot(path=str(screenshot_path), full_page=True)
        return PatientResult(patient_number, medications, False, None, None, find_results_panel(page).inner_text(), f"timed out waiting for results to settle: {error}", screenshot_path)
...
def run_all_patients() -> list[PatientResult]:
    ...
    with sync_playwright() as playwright:
        ...
        for patient_number, medications in PATIENTS:
            ...
            find_medication_field(page).wait_for(state="visible", timeout=SETTLE_TIMEOUT_MS)
...
def format_patient_section(result: PatientResult) -> str:
    ...
    lines = [
        ...
        "",
        f"**Screenshot:** `{result.screenshot_path.relative_to(RESULTS_DIR.parent.parent)}`",
    ]
...
def build_report(results: list[PatientResult]) -> str:
    ...
    total_drugs = sum(len(result.medications.split(",")) for result in results)
    unresolved_total = sum(len((result.response_json or {}).get("unresolved_entries") or []) for result in results if result.response_json)
    resolved_entries = total_drugs - unresolved_total
    total_alerts = sum(len((result.response_json or {}).get("alerts") or []) for result in results if result.response_json)
    header = [
        ...
        f"Entries resolved to a label across all 10 patients: {resolved_entries} of {total_drugs} (compare to 11 of 60 in the original `results/report.md`).",
        f"Interaction alerts fired, any patient: {total_alerts} (compare to 0 in the original report).",
        "",
...
def main() -> None:
    ...
    Gives nothing; writes report.md and raw_responses.json next to this script, and prints to stdout.
    """
